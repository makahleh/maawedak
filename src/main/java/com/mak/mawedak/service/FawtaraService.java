...
public class FawtaraService {
  ...
  public boolean sendInvoicesToFawtara(Invoice invoice, String customerSecretKey, String customerClientKey) {
    ...
    headers.set("secret_key", customerSecretKey);
    headers.set("client_key", customerClientKey);
  ...
  private String buildInvoiceXml(Invoice invoice) {
    ...
    String invoiceHeader = """
            ...
            invoice.getIssueDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
            invoice.getInvoiceIncrementalId());
    ...
    String clientTaxId = invoice.getClientTaxId().isEmpty() ? "" : invoice.getClientTaxId();
    String clientPhoneNumber = invoice.getClientPhoneNumber().isEmpty() ? "" : invoice.getClientPhoneNumber();
    ...
    String sellerSupplierParty = """
            ...
            """.formatted(invoice.getCustomerIncomeSourceSequenceNumber());
    ...
    String legalMonetaryTotal = """
              <cac:LegalMonetaryTotal>
                <cbc:TaxExclusiveAmount currencyID="JO">%.2f</cbc:TaxExclusiveAmount>
                <cbc:TaxInclusiveAmount currencyID="JO">%.2f</cbc:TaxInclusiveAmount>
                <cbc:AllowanceTotalAmount currencyID="JO">0</cbc:AllowanceTotalAmount>
                <cbc:PayableAmount currencyID="JO">%.2f</cbc:PayableAmount>
              </cac:LegalMonetaryTotal>
            """.formatted(
    ...
    for (InvoiceItem item : invoice.getInvoiceItems()) {
      ...
      String line = """
                <cac:InvoiceLine>
                  ...
                  <cbc:LineExtensionAmount currencyID="JO">%.2f</cbc:LineExtensionAmount>
                  <cac:Item>
                    <cbc:Name>%s</cbc:Name>
                  </cac:Item>
                  <cac:Price>
                    <cbc:PriceAmount currencyID="JO">%.2f</cbc:PriceAmount>
                    <cac:AllowanceCharge>
                      <cbc:ChargeIndicator>false</cbc:ChargeIndicator>
                      <cbc:AllowanceChargeReason>DISCOUNT</cbc:AllowanceChargeReason>
              ...
              item.getQuantity(),
              total,
              "خدمات العلاج الطبيعي", // description of the item, can be customized as needed
              item.getUnitPrice());
